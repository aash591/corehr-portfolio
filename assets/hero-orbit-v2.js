(function(){
  ...
  var start=performance.now(),duration=56000,focusAngle=-0.20,lastW=0,radius=185,isMobile=false;
  ...
  function measure(){
    ...
    if(w===lastW&&desk.dataset.mobile===String(isMobile))return;
    lastW=w;desk.dataset.mobile=String(isMobile);
    radius=isMobile?Math.min(124,Math.max(108,(w-118)/2-3)):Math.min(194,Math.max(158,(w-180)/2-7));
    desk.style.setProperty('--orbit-radius',radius+'px');
    desk.style.setProperty('--orbit-diameter',(radius*2)+'px');
  }
  function sizeCard(card,shape){
    var img=card.querySelector(':scope > img'),icon=card.querySelector('.orbit-icon'),title=card.querySelector('b'),sub=card.querySelector('small'),text=card.querySelector(':scope > span');
    var base=isMobile?{w:104,h:53,p:6,r:19,g:6,img:36,ir:11,pt:9,ic:15,it:-8,t:7.5,s:5.5}:{w:146,h:66,p:8,r:25,g:9,img:48,ir:15,pt:12,ic:20,it:-11,t:10,s:7};
    var add=isMobile?{w:35,h:12,p:2,r:7,g:2,img:8,ir:3,pt:3,ic:4,it:-1,t:1.5,s:.7}:{w:38,h:13,p:2,r:8,g:2,img:10,ir:3,pt:3,ic:5,it:-2,t:1.5,s:.8};
    function v(k){return base[k]+add[k]*shape}
    card.style.width=v('w').toFixed(1)+'px';card.style.minHeight=v('h').toFixed(1)+'px';card.style.padding=v('p').toFixed(1)+'px';card.style.borderRadius=v('r').toFixed(1)+'px';card.style.columnGap=v('g').toFixed(1)+'px';
    if(img){img.style.width=v('img').toFixed(1)+'px';img.style.height=v('img').toFixed(1)+'px';img.style.borderRadius=v('ir').toFixed(1)+'px'}
    if(text)text.style.paddingTop=v('pt').toFixed(1)+'px';
    if(icon){icon.style.width=v('ic').toFixed(1)+'px';icon.style.height=v('ic').toFixed(1)+'px';icon.style.top=v('it').toFixed(1)+'px'}
    if(title)title.style.fontSize=v('t').toFixed(2)+'px';if(sub)sub.style.fontSize=v('s').toFixed(2)+'px';
  }
  function paint(now){
    measure();
    ...
    cards.forEach(function(card,i){
      ...
      var shape=smooth01((Math.cos(angle)+1)/2);
      var x=Math.cos(angle)*radius,y=Math.sin(angle)*radius;
      var jiggle=reduce?0:Math.sin(now/1150+i*1.67)*.9,float=reduce?0:Math.sin(now/1550+i*2.03)*1.4,scale=1+focus*.09;
      sizeCard(card,shape);
      ...
      if(more){more.style.opacity=focus.toFixed(3);more.style.maxHeight=(focus*16).toFixed(1)+'px';more.style.marginTop=(focus*2).toFixed(1)+'px'}
