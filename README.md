...
The website uses local WebP images. Service photos are in
`assets/images/services/`, and supporting photos are in `assets/images/`.
Service-image prompts are recorded in `tools/service-image-prompts.json`.
These are illustrative business images, not photographs of Core HR employees
or clients.

The hero image loads immediately; images further down the page lazy-load.
When replacing a photo, update its `src`, dimensions and descriptive `alt`
text in `index.html`. Remove superseded images once no page or tool uses them.
Old service icons, duplicate PNG/JPG originals and unused decorative
illustrations have been removed from the active site directory.
...
section and the "Why Core HR" grid are the natural places.
...
- **Sections.** Eyebrow labels use names without decorative numbering.
  If adding or removing a section, update desktop and mobile navigation,
  relevant footer links and the scroll-spy array in the script.
